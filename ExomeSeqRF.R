...
rf_output_total <- randomForest(demographics$phenotype[non.list]~.,data=data.frame(exome_variants_diff_complete),proximity=TRUE, keep.forest=T,ntree=100)
...
exome_variants_rf_selected_AMRvsAll<-exome_variants_diff_complete[,fit$varselect.interp]
...
df_joint_sign<-cbind(df_joint_qc[id.joint,],Diff.AMR,Diff.CMR,Diff.NoRej,OR,p.value.OR,rf_output$importance)
write.table(df_joint_sign,file="/Users/Pinedasans/Documents/Catalyst/Results/ResultsRF_2steps.txt",sep="\t",row.names = F)
